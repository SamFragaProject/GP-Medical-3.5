...
const TAB_COLORS: Record<string, { gradient: string; bg: string; text: string; ring: string }> = {
    general: { gradient: 'from-slate-500 to-gray-600', bg: 'bg-slate-800/60 backdrop-blur-md', text: 'text-white', ring: 'ring-slate-500/50' },
    laboral: { gradient: 'from-blue-500 to-indigo-600', bg: 'bg-blue-900/40 backdrop-blur-md', text: 'text-blue-200', ring: 'ring-blue-500/50' },
    contacto: { gradient: 'from-violet-500 to-purple-600', bg: 'bg-violet-900/40 backdrop-blur-md', text: 'text-violet-200', ring: 'ring-violet-500/50' },
    expediente: { gradient: 'from-teal-500 to-emerald-600', bg: 'bg-teal-900/40 backdrop-blur-md', text: 'text-teal-200', ring: 'ring-teal-500/50' },
    recetas: { gradient: 'from-pink-500 to-rose-600', bg: 'bg-pink-900/40 backdrop-blur-md', text: 'text-pink-200', ring: 'ring-pink-500/50' },
    dictamenes: { gradient: 'from-cyan-500 to-blue-600', bg: 'bg-cyan-900/40 backdrop-blur-md', text: 'text-cyan-200', ring: 'ring-cyan-500/50' },
    incapacidades: { gradient: 'from-red-500 to-rose-600', bg: 'bg-red-900/40 backdrop-blur-md', text: 'text-red-200', ring: 'ring-red-500/50' },
}

// ─── Expediente section cards for the main landing ───
const EXPEDIENTE_SECTIONS = [
    { value: 'expediente', label: 'Expediente Clínico', icon: Stethoscope, desc: 'Historia, APNP, AHF, Exploración', gradient: 'from-teal-500 to-emerald-600', glow: '#10b981', tint: 'rgba(16,185,129,0.08)', group: 'clinico' as const },
    { value: 'general', label: 'Datos Personales', icon: User, desc: 'Nombre, CURP, RFC, NSS', gradient: 'from-slate-400 to-slate-600', glow: '#94a3b8', tint: 'rgba(148,163,184,0.06)', group: 'info' as const },
    { value: 'laboral', label: 'Datos Laborales', icon: Building2, desc: 'Empresa, puesto, riesgos IA', gradient: 'from-blue-500 to-indigo-600', glow: '#3b82f6', tint: 'rgba(59,130,246,0.08)', group: 'info' as const },
    { value: 'recetas', label: 'Recetas', icon: Pill, desc: 'Prescripciones médicas activas', gradient: 'from-pink-500 to-rose-600', glow: '#ec4899', tint: 'rgba(236,72,153,0.07)', group: 'diagnostico' as const },
    { value: 'dictamenes', label: 'Dictámenes', icon: FileText, desc: 'Aptitud laboral y certificados', gradient: 'from-cyan-500 to-blue-600', glow: '#06b6d4', tint: 'rgba(6,182,212,0.08)', group: 'diagnostico' as const },
    { value: 'incapacidades', label: 'Incapacidades', icon: FileBarChart, desc: 'Certificados de incapacidad', gradient: 'from-red-500 to-rose-600', glow: '#ef4444', tint: 'rgba(239,68,68,0.06)', group: 'diagnostico' as const },
    { value: 'contacto', label: 'Contacto / Foto', icon: Phone, desc: 'Teléfono, email, fotografía', gradient: 'from-violet-500 to-purple-600', glow: '#8b5cf6', tint: 'rgba(139,92,246,0.07)', group: 'info' as const },
]
...
function EditableField({ icon: Icon, label, value, field, type = 'text', options, editing, onChange }: {
    ...
    if (!editing) {
        return (
            <div className="group relative flex items-start gap-3 p-4 bg-slate-900/40 rounded-2xl border border-white/5 hover:bg-slate-800/60 hover:border-white/10 transition-all shadow-sm">
                <div className="flex items-center justify-center w-10 h-10 flex-shrink-0 rounded-xl bg-slate-800 border border-white/5 group-hover:bg-slate-700 transition-colors shadow-inner">
                    <Icon className="w-4 h-4 text-emerald-400 group-hover:text-emerald-300 drop-shadow-sm" />
                </div>
                <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-slate-400 mb-1 transition-colors">{label}</p>
                    <p className={`text-sm font-semibold text-slate-200 truncate group-hover:text-white transition-colors ${type === 'mono' ? 'font-mono tracking-wider text-emerald-100' : ''}`}>{value || '—'}</p>
                </div>
            </div>
        )
    }
    ...
    if (options) {
        return (
            <div className="flex items-start gap-3 p-4 bg-emerald-900/20 rounded-2xl border border-emerald-500/30 transition-all shadow-[0_0_15px_rgba(16,185,129,0.1)]">
                <div className="flex items-center justify-center w-10 h-10 flex-shrink-0 rounded-xl bg-emerald-500/20 border border-emerald-500/30 shadow-inner">
                    <Icon className="w-4 h-4 text-emerald-400 drop-shadow-md" />
                </div>
                <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-1.5">{label}</p>
                    <select
                        value={value || ''}
                        onChange={(e) => onChange(field, e.target.value)}
                        className="w-full bg-slate-900 border border-emerald-500/30 rounded-lg px-3 py-2 text-sm font-semibold text-emerald-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all"
                    >
    ...
    return (
        <div className="flex items-start gap-3 p-4 bg-emerald-900/20 rounded-2xl border border-emerald-500/30 transition-all shadow-[0_0_15px_-3px_rgba(16,185,129,0.15)] relative">
            <div className="flex items-center justify-center w-10 h-10 flex-shrink-0 rounded-xl bg-emerald-500/20 border border-emerald-500/30 shadow-inner">
                <Icon className="w-4 h-4 text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.8)]" />
            </div>
            <div className="flex-1 min-w-0">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-1.5">{label}</p>
                <input
                    ...
                    value={value || ''}
                    onChange={(e) => onChange(field, e.target.value)}
                    className="w-full bg-slate-900/80 border border-emerald-500/30 rounded-lg px-3 py-2 text-sm font-semibold text-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all placeholder:text-slate-500"
                    placeholder={label}
                />
            </div>
...
export default function PerfilPaciente() {
    ...
    // ⚡ /midu — Videogame-style screen navigation direction
    const [navDirection, setNavDirection] = useState<'forward' | 'back'>('forward')

    // Navigation helpers — set direction BEFORE state change
    const navigateForward = (cat: 'info' | 'clinico' | 'diagnostico', tab: string) => {
    ...
    // Slide animation variants — GPU-accelerated transforms only
    const slideVariants = {
        enterFromRight: { opacity: 0, x: 280, scale: 0.97 },
        enterFromLeft: { opacity: 0, x: -280, scale: 0.97 },
        center: { opacity: 1, x: 0, scale: 1 },
        exitToLeft: { opacity: 0, x: -200, scale: 0.97 },
        exitToRight: { opacity: 0, x: 200, scale: 0.97 },
    }
    const transitionConfig = { duration: 0.35, ease: [0.25, 0.46, 0.45, 0.94] as [number, number, number, number] }
    ...
    // ── ELITE DARK THEME — Aplica clase especial ──
    useEffect(() => {
        document.body.classList.add('perfil-elite-dark')
        return () => {
            document.body.classList.remove('perfil-elite-dark')
        }
    }, [])
    ...
    // ── Action dropdown state ── (debe estar ANTES de cualquier return early)
    ...
    return (
        <div className="layout-perfil-paciente space-y-6 pb-12 p-6 sm:p-10">
            {/* ── BREADCRUMB OSCURO INLINE ── */}
            <nav className="flex items-center gap-2 text-xs font-bold text-white/30 mb-2 self-start">
                <button onClick={() => navigate('/dashboard')} className="flex items-center gap-1 hover:text-white/60 transition-colors">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                </button>
                <span className="text-white/20">›</span>
                <button onClick={() => navigate('/pacientes')} className="hover:text-white/60 transition-colors">Pacientes</button>
                <span className="text-white/20">›</span>
            ...
            >
                <div className="p-6 sm:p-10">
                    ...
                    <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-2 mt-4">
                                {[
                                    ...
                                    paciente.estatus && { icon: paciente.estatus === 'activo' ? '✅' : '⏸️', text: paciente.estatus, bg: paciente.estatus === 'activo' ? 'bg-[#00ffb2]/10 text-[#00ffb2]' : 'bg-slate-500/10' },
                        ...
                        {/* Quick info cards */}
                        <div className="flex flex-wrap xl:grid xl:grid-cols-2 gap-3 shrink-0 mt-6 lg:mt-0">
                            {[
                                paciente.empresa_nombre && { icon: Building2, label: 'Empresa', text: paciente.empresa_nombre, color: 'text-blue-400', glow: 'bg-blue-500/10 border-blue-500/20' },
                                paciente.puesto && { icon: Briefcase, label: 'Puesto', text: paciente.puesto, color: 'text-amber-400', glow: 'bg-amber-500/10 border-amber-500/20' },
                                paciente.curp && { icon: Shield, label: 'CURP', text: paciente.curp, color: 'text-emerald-400', glow: 'bg-emerald-500/10 border-emerald-500/20' },
                                paciente.rfc && { icon: FileText, label: 'RFC', text: paciente.rfc, color: 'text-purple-400', glow: 'bg-purple-500/10 border-purple-500/20' },
                                paciente.nss && { icon: Heart, label: 'NSS (IMSS)', text: paciente.nss, color: 'text-rose-400', glow: 'bg-rose-500/10 border-rose-500/20' },
                                paciente.telefono && { icon: Phone, label: 'Contacto', text: paciente.telefono, color: 'text-cyan-400', glow: 'bg-cyan-500/10 border-cyan-500/20' },
                                ...
                                <motion.div
                                    ...
                                    className={`px-4 py-3 rounded-2xl ${card.glow} border backdrop-blur-sm flex items-center gap-3 min-w-[210px] hover:scale-[1.02] transition-transform shadow-lg cursor-default`}
                                >
                                    <div className={`p-2 rounded-xl bg-slate-950/40 ${card.color} shadow-inner`}>
                                        <card.icon className="w-5 h-5 drop-shadow-md" />
                                    </div>
                                    <div className="flex flex-col flex-1 min-w-0">
                                        <p className="text-[10px] uppercase tracking-widest font-black text-slate-400/80 mb-0.5">{card.label}</p>
                                        <p className="text-sm font-bold text-white truncate w-full">{card.text}</p>
                                    </div>
                                </motion.div>
                            ))}
                        </div>
            ...
            {(() => {
                ...
                return (
                    <>
                        {/* overflow-hidden prevents horizontal scrollbar during slide transitions */}
                        <div className="overflow-hidden">
                        <AnimatePresence mode="wait" initial={false}>
                            {/* ═══ LEVEL 0: EXPEDIENTE LANDING — Spectacular Cards + Dashboard ═══ */}
                            {!activeCategory && (
                                <motion.div
                                    key="expediente-landing"
                                    initial={navDirection === 'back' ? slideVariants.enterFromLeft : slideVariants.enterFromRight}
                                    animate={slideVariants.center}
                                    exit={navDirection === 'forward' ? slideVariants.exitToLeft : slideVariants.exitToRight}
                                ...
                                >
                                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                        ...
                                        {/* ═══ RIGHT: Premium Clinical Dashboard ═══ */}
                                        <div className="lg:col-span-5 xl:col-span-4">
                                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2, duration: 0.5 }}>
                                                <Suspense fallback={
                                                    <div className="flex flex-col items-center justify-center py-16 gap-3 rounded-[1.5rem] border border-slate-100 bg-white">
                                                        <Loader2 className="w-8 h-8 animate-spin text-emerald-500" />
                                                        <span className="text-xs text-slate-400 font-bold">Cargando dashboard...</span>
                                                    </div>
                                                }>
                                                    <PatientDashboardTab pacienteId={id!} onNavigate={(tab) => {
                                                        const clinicalModules = ['laboratorio', 'audiometria', 'espirometria', 'electrocardiograma', 'vision', 'rayosx']
                                                        const tabMap: Record<string, string> = { laboratorio: 'laboratorio', audiometria: 'audiometria', espirometria: 'espirometria', electrocardiograma: 'electrocardiograma', vision: 'vision', rayosx: 'rayosx' }
                                                        navigateForward('clinico', tabMap[tab] || tab)
                                                    }} />
                                                </Suspense>
                                            </motion.div>
                                        </div>
                            ...
                            {activeCategory && (
                                <motion.div
                                    ...
                                    initial={navDirection === 'forward' ? slideVariants.enterFromRight : slideVariants.enterFromLeft}
                                    animate={slideVariants.center}
                                    exit={navDirection === 'back' ? slideVariants.exitToRight : slideVariants.exitToLeft}
                                    transition={transitionConfig}
                                    className="space-y-6"
                                >
                                        ...
                                        <div className="p-5 sm:p-7 relative z-10">
                                            ...
                                            <div className="flex items-center gap-4 mb-6 relative z-10">
                                                <motion.button
                                                    ...
                                                    className={`flex items-center justify-center w-11 h-11 rounded-2xl bg-${catColor}-500/10 border border-${catColor}-500/30 hover:bg-${catColor}-500/20 text-${catColor}-400 transition-all shadow-inner`}
                                                >
                                                    <ArrowLeft className="w-5 h-5 drop-shadow-md" />
                                                </motion.button>
                                                <div className="flex-1">
                                                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400/80 mb-1">← Vista General</p>
                                                    <p className={`text-2xl font-extrabold text-white tracking-tight drop-shadow-md`}>{catLabel}</p>
                                                </div>
                                            </div>
                                                ...
                                                {TABS.filter(t => t.group === activeCategory).map((tab, i) => {
                                                    ...
                                                    return (
                                                        <motion.button
                                                            ...
                                                            className={`group relative flex items-center gap-3 px-4 py-3 rounded-2xl border transition-all duration-200 cursor-pointer
                                                                ${isActive ? `${c.bg} ${c.ring} shadow-[0_4px_20px_rgba(0,0,0,0.3)] border-${c.ring.split('-')[1]}-500/30` : 'bg-slate-900/40 border-white/5 hover:bg-slate-800/60 hover:border-white/10'}`}
                                                        >
                                                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all
                                                                ${isActive ? `bg-gradient-to-br ${c.gradient} shadow-lg shadow-${c.ring.split('-')[1]}-500/20` : 'bg-white/5 border border-white/10 group-hover:border-white/20'}`}>
                                                                <tab.icon className={`w-4 h-4 ${isActive ? 'text-white drop-shadow-md' : 'text-slate-400 group-hover:text-white'}`} />
                                                            </div>
                                                            <span className={`text-sm font-bold tracking-wide ${isActive ? c.text : 'text-slate-400 group-hover:text-white'}`}>{tab.label}</span>
                                                        </motion.button>
                                                    )
                                                })}
                                            </div>
                                    ...
                                    {activeTabConfig?.description && (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            className="flex items-center gap-3 px-5 py-3.5 bg-slate-900/60 backdrop-blur-xl rounded-[1.25rem] border border-white/10 shadow-[0_4px_15px_rgba(0,0,0,0.3)] w-max max-w-full overflow-hidden"
                                        >
                                            <activeTabConfig.icon className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                                            <span className="text-[13px] font-semibold text-slate-300 relative z-10 truncate">{activeTabConfig.description}</span>
                                        </motion.div>
                                    )}
                                        ...
                                        <AnimatePresence mode="wait">
                                            ...
                                            >
                                                ...
                                                <TabsContent value="general" className="space-y-6 mt-0">
                                                    ...
                                                    <Card className="border-amber-500/30 bg-amber-900/20 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.3)] rounded-[2rem] overflow-hidden">
                                                        <CardHeader className="pb-2">
                                                            <CardTitle className="flex items-center gap-2 text-amber-400 text-base font-bold tracking-wide">
                                                ...
                                                <TabsContent value="laboral" className="space-y-6 mt-0">
                                                    ...
                                                    {/* ═══ RISK ANALYZER ═══ */}
                                                    <Card className="border-violet-500/30 bg-gradient-to-br from-violet-900/20 to-indigo-900/20 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.4)] rounded-[2rem] overflow-hidden relative">
                                                        <div className="absolute top-0 right-0 w-64 h-64 bg-violet-500/10 rounded-full blur-[60px] pointer-events-none -translate-y-1/2 translate-x-1/3" />
                                                        <CardHeader className="pb-4 relative z-10">
                                                            <div className="flex items-center justify-between">
                                                                <CardTitle className="flex items-center gap-3 text-violet-300 text-base font-bold tracking-wide">
                                                                    <div className="p-2 bg-violet-500/20 rounded-xl border border-violet-500/30 shadow-inner">
                                                                        <Brain className="w-5 h-5 text-violet-400 drop-shadow-[0_0_8px_rgba(139,92,246,0.8)]" />
                                                                    </div>
                                                                    Analizador de Riesgos Laborales
                                                                    <Badge className="bg-violet-900/50 text-violet-300 border border-violet-500/40 text-[10px] uppercase font-black tracking-widest px-2 py-0.5 shadow-[0_0_10px_rgba(139,92,246,0.2)]">IA</Badge>
                                                                </CardTitle>
                                                                <div className="flex gap-3">
                                                                    <Button onClick={handleAnalyzeRisks} disabled={isAnalyzingAI} size="sm" className="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 shadow-[0_4px_15px_-3px_rgba(139,92,246,0.5)] border border-violet-400/20 text-white rounded-xl gap-2 font-bold px-4 transition-all hover:scale-[1.02]">
                                                                        {isAnalyzingAI ? (<><Loader2 className="w-4 h-4 animate-spin" /> Analizando...</>) : (<><Zap className="w-4 h-4 text-violet-200" /> Analizar con IA</>)}
                                                                    </Button>
                                                                    {showRiesgos && totalRisksCount > 0 && (
                                                                        <Button onClick={saveRisksOnly} disabled={saving} size="sm" variant="outline" className="border-emerald-500/40 bg-emerald-900/20 text-emerald-400 hover:bg-emerald-500/30 hover:border-emerald-400 rounded-xl gap-2 font-bold transition-all hover:scale-[1.02] shadow-[0_4px_15px_-3px_rgba(16,185,129,0.2)]">
