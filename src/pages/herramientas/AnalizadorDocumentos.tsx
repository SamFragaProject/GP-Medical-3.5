...
export default function AnalizadorDocumentos() {
    ...
    const canGoNext = () => {
        if (step === 'upload') return stats.total > 0;
        if (step === 'results') return stats.completed > 0;
        if (step === 'verify') return allParams.length > 0;
        if (step === 'charts') return true;
        return false;
    };
    const handleNext = () => { if (stepIdx < STEPS.length - 1) setStep(STEPS[stepIdx + 1].id); };
    const handlePrev = () => { if (stepIdx > 0) setStep(STEPS[stepIdx - 1].id); };
    ...
    return (
        <div className="min-h-[80vh] flex flex-col">
            {/* ── HEADER (WizardAltaPaciente style) ── */}
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 mb-6 border border-white/10">
                ...
                <div className="relative z-10 flex items-center justify-between mb-6">
                    <div>
                        <h1 className="text-2xl font-black text-white mb-1">MedExtract Pro</h1>
                        <p className="text-sm text-slate-400">Motor de extracción médica con IA — Gemini + OpenAI</p>
                    </div>
                    <div className="flex gap-2">
                        {stats.completed > 0 && <>
                            <button onClick={exportToCsv} className="px-4 py-2 bg-white/10 text-white rounded-xl text-xs font-bold hover:bg-white/20 flex items-center gap-2"><Download className="w-3.5 h-3.5" /> CSV</button>
                ...
                <div className="relative z-10 flex items-center justify-between gap-2">
                    {STEPS.map((s) => {
                        ...
                        const isCompleted = stepIdx > STEPS.findIndex(x => x.id === s.id);
                        const StepIcon = s.icon;
                        const COLORS: Record<string, string> = {
                            upload: 'from-emerald-500 to-teal-600',
                            results: 'from-blue-500 to-indigo-600',
                            verify: 'from-violet-500 to-purple-600',
                            charts: 'from-amber-500 to-orange-600',
                            usage: 'from-rose-500 to-pink-600',
                        };
                        return (
                            <button key={s.id} onClick={() => { const idx = STEPS.findIndex(x => x.id === s.id); if (idx < stepIdx) setStep(s.id); }}
                                ...
                                <div className="hidden lg:block text-left min-w-0">
                                    <p className={`text-[10px] font-bold uppercase tracking-wider truncate ${isActive ? 'text-white' : isCompleted ? 'text-emerald-400' : 'text-slate-500'}`}>{s.label}</p>
                                </div>
                            </button>
                        );
                    })}
                ...
                <div className="relative z-10 mt-4 h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <motion.div className="h-full bg-gradient-to-r from-emerald-400 to-teal-500 rounded-full" animate={{ width: `${progress}%` }} transition={{ duration: 0.4, ease: 'easeOut' }} />
                </div>
            </div>

            {/* Step info banner */}
            {STEP_INFO[step] && (
                <div className="p-4 mb-4 bg-gradient-to-r from-violet-50 to-indigo-50 border border-violet-200/50 rounded-2xl flex items-start gap-3">
            ...
            <div className="flex items-center justify-between mt-6 p-4 bg-white rounded-2xl shadow-lg border border-slate-100">
                <Button variant="outline" onClick={stepIdx === 0 ? undefined : handlePrev} disabled={stepIdx === 0} className="h-11 px-6 rounded-xl gap-2">
                    <ArrowLeft className="w-4 h-4" /> {stepIdx === 0 ? 'Inicio' : 'Anterior'}
                </Button>
                <div className="flex items-center gap-2">
                    {STEPS.map(s => (
