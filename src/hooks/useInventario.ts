  ...
  const fetchInitialData = useCallback(async () => {
    ...
    try {
      ...
      const mappedProducts: Producto[] = data.map((p: any) => ({
        ...
        tipo: (p.categoria === 'medicamento' ? 'medicamento' : 'consumible') as any,
        categoria: (p.categoria as any) || 'antisepticos',
        ...
        stock: {
          ...
          ubicacion: p.ubicacion || 'Bodega Central',
          fechaEntrada: p.created_at ? new Date(p.created_at) : new Date(),
