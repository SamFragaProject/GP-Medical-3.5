...
export type VisualizationType = 'gauge' | 'bar_chart' | 'simple' | 'list' | 'radar' | 'line_chart' | 'table_row';
...
export interface LabResult {
  name: string;
  value: string | number | string[] | RadarPoint[];
  unit: string;
  range: string;
  description?: string;
...
export type MedicalSectionId = keyof typeof MEDICAL_SECTIONS;
export type MedicalSectionName = typeof MEDICAL_SECTIONS[MedicalSectionId];

export interface ReportSectionData {
    name: MedicalSectionName;
    summary?: string;
    results: LabResult[];
}
...
export interface MedicalReport {
  ...
  sections: Partial<Record<MedicalSectionId, ReportSectionData>>;
}

// =================================================================
// NUEVA ESTRUCTURA PARA HISTORIA CLÍNICA - GP MEDICAL HEALTH
// =================================================================
...
export interface ClinicalHistoryFormData {
    ...
    estudiosComplementarios: {
        ...
        espirometria: {
            ...
            fvc: string;
            reporte: string;
            flowVolumeCurve?: { x: number[]; y: number[]; pred_x?: number[]; pred_y?: number[]; };
            volumeTimeCurve?: { x: number[]; y: number[]; pred_x?: number[]; pred_y?: number[]; };
        };
        optometria: {
            realizada: 'SI' | 'NO' | '';
            resultado: 'Normal' | 'Anormal' | '';
...
export const getInitialClinicalHistoryFormData = (): ClinicalHistoryFormData => {
    const heredofamiliaresKeys = ['ARTRITIS', 'CÁNCER', 'CARDIOPATÍAS', 'DEPRESIÓN', 'DIABETES MELLITUS', 'OBESIDAD', 'PRESIÓN ARTERIAL ALTA', 'OTRO'];
    const personalesPatologicosKeys = [
    ...
    const audiometriaFrequencies = ['125', '250', '500', '1000', '2000', '3000', '4000', '6000', '8000'];
    ...
    return {
        ...
        antecedentes: {
            heredofamiliares: heredofamiliaresKeys.reduce((acc, key) => ({ ...acc, [key]: { has: '', parentescos: { padre: false, madre: false, hermanos: false, otros: false }, especifique: '' } }), {}),
            personalesNoPatologicos: {
                vacunacion: '',
                tabaquismo: { has: '', numeroCigarros: '', frecuencia: '' },
                alcoholismo: { has: '', frecuencia: '' },
            },
            personalesPatologicos: personalesPatologicosKeys.reduce((acc, key) => ({ ...acc, [key]: { has: 'NO', especifique: '' } }), {}),
        ...
        exploracionFisica: {
            ...
            examenSistemas: examenSistemasKeys.reduce((acc, key) => ({ ...acc, [key]: { status: 'NORMAL', comentarios: '' } }), {}),
        },
        estudiosComplementarios: {
            audiometria: { realizada: '', resultado: '', reporte: audiometriaFrequencies.reduce((acc, freq) => ({ ...acc, [freq]: { derecho: '', izquierdo: '' } }), {}) },
            espirometria: { realizada: '', resultado: '', predicho_fvc: '', predicho_fev1: '', fev1: '', fvc: '', reporte: '', flowVolumeCurve: { x: [], y: [] }, volumeTimeCurve: { x: [], y: [] } },
...
export interface AptitudeCertificateData {
  ...
  company: string;
  observations: string[];
  validityPeriod: '3 MESES' | '6 MESES' | '1 AÑO' | 'OTRO';
  nextEvaluationDate: {
    day: string;
    month: string;
    year: string;
  };
  examLocationAndDate: string;
  doctorName: string;
...
export type ExportProfile = 'dsv' | 'ims_gear_limited' | 'ims_gear_all';

export interface PdfExportOptions {
  selections: Record<string, boolean>; // a map of sectionId or result.name to boolean
  grayscale: boolean;
  layout: 'list' | 'grid';
...
export interface CertificateConfig {
    fitnessStatus?: 'APTO' | 'APTO CON RESTRICCIONES' | 'NO APTO';
    validityPeriod?: '3 MESES' | '6 MESES' | '1 AÑO' | 'OTRO';
