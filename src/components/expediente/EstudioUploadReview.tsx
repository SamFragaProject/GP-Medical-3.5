...
export default function EstudioUploadReview({ pacienteId, tipoEstudio, pacienteNombre, isCompact, onSaved }: Props) {
    ...
    const startProcess = async (files: File[]) => {
        ...
        try {
            ...
            const { error: uploadError } = await supabase.storage.from('documentos-medicos').upload(fileName, primaryFile)
            ...
            for (const f of files) {
                const prep = await prepareFilesForAnalysis(f)
                rawText += prep.rawText + "\n\n"
                images.push(...prep.images)
            }
            setProgress(50)

            const file = primaryFile // For waveform extraction compatibility
            ...
            return (
                <div className="max-w-xl mx-auto w-full">
                    <div className={`p-10 border-4 border-dashed rounded-[3rem] transition-all flex flex-col items-center justify-center gap-6 cursor-pointer ${isDragging ? 'border-emerald-500 bg-emerald-50 animate-pulse' : 'border-slate-100 hover:border-emerald-400 hover:bg-emerald-50/20'}`}
                        onDrop={(e) => { e.preventDefault(); const files = Array.from(e.dataTransfer.files); if (files.length > 0) { setSelectedFile(files[0]); startProcess(files); } }}
            ...
            return (
                <div className="max-w-xl mx-auto w-full">
                    <Card className="border-0 shadow-2xl p-16 bg-white rounded-[4rem] text-center space-y-8 animate-in zoom-in-95 duration-500">
                        ...
                        <button
                            onClick={() => { setPhase('idle'); setExtractedData(null); onSaved?.(); }}
    ...
    if (isCompact && phase !== 'idle') {
        return (
            <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-slate-900/60 backdrop-blur-md p-4 md:p-10">
                <div className="relative w-full max-w-5xl mx-auto mt-[5vh] lg:mt-[10vh]">
                    {content}
                </div>
            </div>
        )
