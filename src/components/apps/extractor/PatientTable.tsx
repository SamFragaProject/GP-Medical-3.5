...
export const PatientTable: React.FC<PatientTableProps> = ({ data, onDelete }) => {
    ...
    return (
        <div className="w-full bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col h-[600px] shadow-sm">
            <div className="overflow-auto flex-1 relative">
                <table className="w-full text-sm text-left text-slate-600">
                    <thead className="text-xs text-slate-700 uppercase bg-slate-50/80 sticky top-0 z-30 shadow-sm backdrop-blur-sm">
                        <tr>
                            <th scope="col" className="px-4 py-3 bg-slate-50 sticky left-0 z-30 border-r border-slate-200 w-12 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                                #
                            </th>
                            <th scope="col" className="px-2 py-3 bg-slate-50 sticky left-12 z-30 border-r border-slate-200 w-[40px] text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                                <Trash2 className="w-4 h-4 mx-auto text-slate-400" />
                            </th>
                            {headers.map((header) => (
                                <th key={header.key} scope="col" className={`px-4 py-3 whitespace-nowrap ${header.width} border-b border-slate-200 font-semibold tracking-wide bg-slate-50`}>
                                    {header.label}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {data.map((patient, index) => (
                            <tr key={patient.id} className="bg-white hover:bg-slate-50/50 transition-colors">
                                <td className="px-4 py-3 font-medium text-slate-900 sticky left-0 bg-white hover:bg-slate-50/50 border-r border-slate-100 z-20 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.03)] text-xs">
                                    {data.length - index}
                                </td>
                                <td className="px-2 py-3 sticky left-12 bg-white hover:bg-slate-50/50 border-r border-slate-100 z-20 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.03)]">
                                    <button
                                        onClick={() => onDelete(patient.id)}
                                        className="text-slate-400 hover:text-red-600 transition-colors p-1.5 rounded-md hover:bg-red-50 group"
                                ...
                                {headers.map((header) => (
                                    <td key={`${patient.id}-${header.key}`} className="px-4 py-3 truncate max-w-[300px] text-slate-600 border-b border-slate-50">
                                        <div className="truncate" title={patient[header.key] || ''}>
                                            {patient[header.key] || <span className="text-slate-300 text-xs">-</span>}
                                        </div>
                                    </td>
                                ))}
                            </tr>
            ...
            <div className="bg-slate-50 px-4 py-3 border-t border-slate-200 text-xs text-slate-500 flex justify-between items-center flex-shrink-0">
                <span className="font-medium text-slate-600">Total: {data.length} pacientes</span>
                <span className="text-slate-400 flex items-center gap-1">
                    Desplázate → para ver todas las columnas
                </span>
            </div>
        </div>
    );
