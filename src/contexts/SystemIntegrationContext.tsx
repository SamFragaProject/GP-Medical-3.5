...
export function SystemProvider({ children }: SystemProviderProps) {
  ...
  const updatePermissions = useCallback((permissions: PermissionCacheState) => {
    ...
    if (stateRef.current.config.enableAuditLogging) {
      logAction({
        action: 'PERMISSION_CHECK',
        resourceType: 'system',
        actionType: 'permissions_update',
        details: {
          previousPermissions: stateRef.current.permissions.permissions,
  ...
  }, [invalidateUserCache, invalidatePermissionsCache, logAction])
    ...
    try {
      ...
      if (currentUser?.enterpriseId) {
        const { data: modulesData, error: modulesError } = await supabase
          ...
          `)
          .eq('empresa_id', currentUser.enterpriseId)
          .eq('status', 'active')

        if (!modulesError && modulesData) {
          // @ts-ignore - Supabase type inference for joins can be tricky
          activeModules = modulesData.map(m => m.system_modules?.key).filter(Boolean)
        }
      }
  ...
  }, [currentUser, getUserPermissions, updatePermissions, updateSession])
  ...
  useEffect(() => {
    ...
    // Sincronizar al montar (solo una vez cuando currentUser aparece)
  ...
  const value = React.useMemo<SystemContextType>(() => ({
  ...
  }), [state, updatePermissions, updateSession, updateNavigation, updateActivity, invalidateCache, syncWithServer, handlePermissionChange])
...
export function useSystemIntegration() {
  ...
  return React.useMemo(() => ({
    ...
    // Check de plugins (Verificación DB)
    isModuleActive: (moduleKey: string) => {
      // Si es super admin, tiene acceso a todo
      if (currentUser?.hierarchy === 'super_admin') return true;

      // Verificar en estado sincronizado (Real from DB)
      return state.permissions.activeModules?.includes(moduleKey) ?? false
    }
  }), [state, currentUser, isSessionActive, isCacheExpired, cacheAge, actions])
}
