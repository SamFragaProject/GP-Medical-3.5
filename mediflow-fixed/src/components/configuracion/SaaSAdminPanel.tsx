...
function SecurityStatus() {
  const { canViewAuditLogs } = useSaaSPermissions()
...
export function SaaSAdminPanel() {
  ...
  const { hasPermission, canViewAuditLogs, getUserHierarchy } = useSaaSPermissions()
  ...
  // Si no tiene permisos básicos, mostrar mensaje
  if (!hasPermission('users')) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">No tienes permisos para ver el panel de administración.</p>
        </div>
      </div>
    )
  }
