...
// import { useCurrentUser } from '@/hooks/useCurrentUser' // Comentado para modo demo
// import { usePermissionCheck } from '@/hooks/usePermissionCheck'

// Mock para signOut en modo demo
const signOut = () => {
  console.log('SignOut called - modo demo')
}
...
export function MenuPersonalizado({ className = '' }: MenuPersonalizadoProps) {
  ...
  // Usuario demo para modo sin autenticación
  const user = {
    id: 'demo-user',
    name: 'Usuario Demo',
  ...
  // Mock de canAccess que siempre permite acceso
  ...
  const generateMenuSections = (): Array<{ id: string; title: string; items: NavigationItem[] }> => {
    ...
    const availableItems: NavigationItem[] = [
      ...
      ...(user?.hierarchy === 'super_admin' || user?.hierarchy === 'admin_empresa' ? [{
        id: 'configuracion',
        name: 'Configuración',
        href: '/configuracion',
        icon: 'Settings',
  ...
  // Cargar items de menú del usuario (modo demo)
  useEffect(() => {
    const loadUserMenuItems = async () => {
      try {
        setLoading(true)
        
        // Modo demo - usar configuración estática
        console.log(`✅ Menú cargado para usuario ${user.hierarchy} (modo demo)`)
      ...
      <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-primary/10 to-primary/5">
        ...
        <div className="mt-2 flex items-center justify-between">
          <Badge variant="secondary" className="text-xs">
            {user?.hierarchy?.replace('_', ' ').toUpperCase() || 'USUARIO'}
          </Badge>
          <button
...
export function MenuHierarchyIndicator() {
  ...
  const loading = false
  const level = user?.hierarchy || 'paciente'
