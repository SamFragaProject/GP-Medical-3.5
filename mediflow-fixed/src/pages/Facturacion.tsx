...
export function Facturacion() {
  ...
  const canManageBilling = useRequirePermission('manage_billing')
  const [pacienteSeleccionado, setPacienteSeleccionado] = useState<any>(null)
  const [modalFacturacionAbierta, setModalFacturacionAbierta] = useState(false)

  // Verificar si es usuario médico
  const isMedico = user?.hierarchy && ['medico_trabajo', 'medico_especialista', 'medico_industrial'].includes(user.hierarchy)
  const canAccessBilling = user?.hierarchy && ['admin_empresa', 'recepcion', 'medico_trabajo', 'medico_especialista'].includes(user.hierarchy)
  ...
  if (isMedico) {
    const medicoInfo = {
      id: user?.id || '',
      nombre: user?.name || '',
      especialidad: user?.metadata?.especialidad || 'Medicina del Trabajo',
      cedula_profesional: user?.metadata?.cedula_profesional || '1234567'
    }

    return (
      <div className="space-y-6">
        <MedicoDashboardFacturacion medico={medicoInfo} />
        ...
        >
          <Button
            onClick={() => setModalFacturacionAbierta(true)}
        ...
        <FacturacionMedicaModal
          open={modalFacturacionAbierta}
          onClose={() => setModalFacturacionAbierta(false)}
          ...
          onFacturaCreada={(factura) => {
            console.log('Factura médica creada:', factura)
            toast.success('Factura médica creada exitosamente')
            setModalFacturacionAbierta(false)
          }}
        />
      </div>
    )
  ...
  // Vista para administradores y recepcionistas
  if (!user || (!canAccessBilling && !canManageBilling)) {
    return (
        ...
        <div className="max-w-md mx-auto text-center">
          <Alert className="mb-6">
            ...
            <AlertDescription>
              ...
              No tienes permisos para gestionar facturación y seguros.
              <br /><br />
