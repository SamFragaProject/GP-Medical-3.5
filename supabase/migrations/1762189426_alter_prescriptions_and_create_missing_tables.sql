-- Migration: alter_prescriptions_and_create_missing_tables
-- Created at: 1762189426

-- Agregar columnas faltantes a tabla recetas
ALTER TABLE recetas 
...
ADD COLUMN IF NOT EXISTS encuentro_id uuid REFERENCES encuentros(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS doctor_id uuid REFERENCES usuarios_perfil(id),
...
-- Actualizar doctor_id si medico_id existe
UPDATE recetas SET doctor_id = medico_id WHERE doctor_id IS NULL AND medico_id IS NOT NULL;
...
ALTER TABLE recetas RENAME COLUMN medico_id TO doctor_id_legacy;
