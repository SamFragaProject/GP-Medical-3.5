...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "image/*", "text/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for lab-results" ON storage.objects FOR SELECT USING (bucket_id = 'lab-results');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for lab-results" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'lab-results');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for lab-results" ON storage.objects FOR UPDATE USING (bucket_id = 'lab-results');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for lab-results" ON storage.objects FOR DELETE USING (bucket_id = 'lab-results');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'lab-results',
        public: true,
        allowed_mime_types: ["application/pdf", "image/*", "text/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"],
