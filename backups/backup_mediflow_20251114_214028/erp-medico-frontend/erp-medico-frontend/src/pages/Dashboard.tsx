...
import { citasPacienteDemo, citasMedicoHoyDemo } from '@/demo/demoData'
...
// El usuario ahora proviene del contexto de autenticación
...
export function Dashboard() {
  ...
  // Filtrar tarjetas según rol para simplificar vista y enfocarse en lo relevante
  const tarjetasResumen = useMemo(() => {
    if (!user) return []
    switch (user.rol) {
      case 'paciente':
        return baseTarjetas.filter(t => ['Mis Citas', 'Mis Exámenes', 'Satisfacción Cliente'].includes(t.titulo))
      case 'medico':
        return baseTarjetas.filter(t => ['Pacientes Activos', 'Citas Hoy', 'Exámenes Completados', 'Tiempo Promedio'].includes(t.titulo))
      case 'admin_empresa':
        return baseTarjetas.filter(t => ['Pacientes Activos', 'Ingresos del Mes', 'Alertas Activas', 'Satisfacción Cliente', 'Tiempo Promedio', 'Equipos Disponibles'].includes(t.titulo))
      case 'super_admin':
      default:
        return baseTarjetas
    }
  ...
  // Vista simplificada para paciente: delegar a DashboardPaciente para evitar duplicidad/confusión
  if (user.rol && (user.rol as any) === 'paciente') {
    return <DashboardPaciente />
  }

  return (
  <div className="space-y-8" data-hc-dashboard>
      {/* Header de bienvenida */}
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="hc-card hc-card-hover p-6"
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {tarjetasResumen.map((tarjeta, index) => (
          <motion.div
            ...
            className="hc-card hc-card-hover overflow-hidden transition-shadow duration-200"
          >
            <div className="p-6">
              <div className="flex items-center justify-between">
      ...
      {/* Vistas rápidas por rol */}
      {user.rol === 'paciente' && (
  <div className="hc-card hc-card-hover p-6">
          <h3 className="font-semibold text-gray-900 mb-4 flex items-center space-x-2">
            <Calendar className="h-4 w-4 text-primary" />
            <span>Próximas citas</span>
          </h3>
          <div className="space-y-3">
            {citasPacienteDemo.map((cita) => (
              <div key={cita.id} className="flex items-center justify-between py-2 border-b last:border-b-0">
                <div>
                  <p className="text-sm font-medium text-gray-900">{cita.motivo}</p>
                  <p className="text-xs text-gray-500">Con {cita.medico} • {cita.hora}</p>
                </div>
                <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">{new Date(cita.fecha).toLocaleDateString('es-ES')}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  <div className="hc-card hc-card-hover p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {citasMedicoHoyDemo.map((cita) => (
              <div key={cita.id} className="p-3 rounded-lg border border-gray-100">
                <p className="text-sm font-medium text-gray-900">{cita.paciente}</p>
                <p className="text-xs text-gray-500">{cita.motivo} • {cita.hora}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div
          ...
          className="lg:col-span-2 hc-card hc-card-hover p-6"
        >
