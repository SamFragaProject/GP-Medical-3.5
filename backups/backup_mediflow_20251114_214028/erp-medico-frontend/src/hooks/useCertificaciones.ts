...
export function useCertificaciones() {
  ...
  useEffect(() => {
    ...
    setCertificaciones(datosEjemplo.certificaciones)
    setPacientes(datosEjemplo.pacientes)
    setEmpresas(datosEjemplo.empresas)
    setTiposCertificado(datosEjemplo.tiposCertificado)
    setAlertas(datosEjemplo.alertas)
  }, [])
  ...
  const obtenerCertificaciones = useCallback(async () => {
    ...
    try {
      ...
      // Los datos ya están cargados por useEffect
    } catch (error) {
  ...
  const crearCertificacion = useCallback(async (datos: Partial<Certificacion>) => {
    try {
      ...
      const numeroCert = generarNumeroCertificado()
      ...
      const nuevaCertificacion: Certificacion = {
        ...
        tipoCertificadoId: datos.tipoCertificadoId!,
        examenOcupacionalId: datos.examenOcupacionalId,
        fechaEmision: new Date().toISOString(),
        fechaVencimiento: calcularFechaVencimiento(datos.tipoCertificadoId!),
        estado: EstadoCertificacion.VIGENTE,
        aptoParaTrabajo: datos.aptoParaTrabajo || false,
        restricciones: datos.restricciones || [],
        recomendaciones: datos.recomendaciones || [],
        observacionesMedicas: datos.observacionesMedicas || '',
        medicoFirma: datos.medicoFirma!,
      ...
      setCertificaciones(prev => [nuevaCertificacion, ...prev])
