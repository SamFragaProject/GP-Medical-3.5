    ...
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {accionesMedicas.map((accion, index) => {
            ...
            return (
              ...
              >
                <Card 
                  className={`cursor-pointer transition-all duration-200 hover:shadow-lg ${accion.borderColor} border-2 hover:${accion.bgColor}`}
                  onClick={() => handleAccionClick(accion)}
                >
                  <CardHeader className="pb-3">
                    <div className={`w-12 h-12 rounded-lg ${accion.bgColor} flex items-center justify-center mb-3`}>
                      <Icon className={`h-6 w-6 ${accion.color}`} />
                    </div>
                    <CardTitle className="text-lg">{accion.titulo}</CardTitle>
