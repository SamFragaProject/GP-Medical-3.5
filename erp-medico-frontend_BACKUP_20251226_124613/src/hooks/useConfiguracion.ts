...
const initialSettings: SettingsState = {
  usuario: [
    {
      ...
      email: 'carlos.mendoza@mediflow.com',
      role: 'Administrador',
    ...
    },
    {
      ...
      email: 'roberto.silva@mediflow.com',
      role: 'Técnico',
      permissions: ['read'],
  ...
  empresa: {
    id: '1',
    name: 'MediFlow',
    legalName: 'MediFlow Sistemas Médicos S.A. de C.V.',
    rfc: 'MSM123456ABC',
    address: {
    ...
    website: 'https://www.mediflow.com',
    isActive: true,
    createdAt: new Date('2024-01-01')
  },
  sedes: [
  ...
  notificaciones: {
    email: {
      ...
      smtpPort: 587,
      username: 'noreply@mediflow.com',
      password: '••••••••',
      fromEmail: 'noreply@mediflow.com',
      fromName: 'MediFlow'
    },
    sms: {
      isEnabled: true,
  ...
  backup: {
    ...
    backupLocation: '/backups/mediflow',
    isActive: true
  },
  auditLogs: [],
  chatbotIA: {
  ...
  reportes: {
    ...
    autoSchedule: false,
    scheduleTime: '08:00',
    emailRecipients: ['admin@mediflow.com']
  },
  seguridad: {
    passwordPolicy: {
      minLength: 8,
...
export function useConfiguracion() {
  ...
  const performBackup = useCallback(() => {
    try {
      ...
      a.href = url
      a.download = `mediflow-backup-${new Date().toISOString().split('T')[0]}.json`
  ...
  const exportSettings = useCallback(() => {
    try {
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `mediflow-config-${new Date().toISOString().split('T')[0]}.json`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
