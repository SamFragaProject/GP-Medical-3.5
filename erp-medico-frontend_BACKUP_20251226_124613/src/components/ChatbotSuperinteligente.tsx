...
export function ChatbotSuperinteligente({ 
  ...
  const enviarMensaje = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Mostrar sugerencias de escalación si es necesario
  ...
  const escalarAHumano = async () => {
    ...
    try {
      const resultado = await chatbot.escalarConversacion(
        conversacionId,
        'Usuario solicitó escalación a agente humano'
      )

      const confirmacionMsg: MensajeChatbot = {
        id: Date.now().toString() + '_confirmacion',
        conversacion_id: conversacionId,
        mensaje: `Perfecto. He creado el ticket ${resultado.numero_ticket} y un agente humano te contactará pronto. Mientras tanto, puedes seguir preguntándome cualquier cosa.`,
        es_usuario: false,
        tipo_mensaje: 'texto',
        created_at: new Date().toISOString()
      }
