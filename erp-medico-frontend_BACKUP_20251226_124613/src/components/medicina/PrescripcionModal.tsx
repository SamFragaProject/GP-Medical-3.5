...
export function PrescripcionModal({ open, onClose, paciente, onPrescripcionCreada }: PrescripcionModalProps) {
  ...
  const [autoCorrecciones, setAutoCorrecciones] = useState<number>(0)
  ...
  // Reconocimiento de voz para diagnóstico
  const {
    isListening,
    start: startVoz,
    stop: stopVoz,
    supported: vozSoportada,
  ...
  const toggleVoz = () => {
    ...
    if (isListening) {
      stopVoz()
      toast.success('Micrófono detenido')
      logAction({ action: 'ROUTE_ACCESS', resourceType: 'prescripcion', actionType: 'VOICE_STOP' })
    } else {
      startVoz()
      toast.success('Escuchando...')
      logAction({ action: 'ROUTE_ACCESS', resourceType: 'prescripcion', actionType: 'VOICE_START' })
    }
  }

  const aplicarCorreccionGramatical = async () => {
    if (!diagnostico.trim()) {
      toast.error('Ingresa un diagnóstico antes de corregir')
      return
    }
    setCorrigiendo(true)
    try {
      ...
      if (matches.length === 0) {
        toast('Sin correcciones sugeridas', { icon: '✳️' })
      } else {
        setDiagnostico(corrected)
        setAutoCorrecciones((c) => c + matches.length)
        toast.success(`Correcciones aplicadas (${matches.length})`)
      }
      logAction({ action: 'RESOURCE_ACCESS', resourceType: 'prescripcion', actionType: 'GRAMMAR_CORRECT', details: { matches: matches.length } })
    } catch (e) {
      toast.error('Error corrigiendo texto')
    } finally {
  ...
  const agregarMedicamento = (medicamento: Omit<Medicamento, 'id'>) => {
    ...
    logAction({ action: 'RESOURCE_ACCESS', resourceType: 'prescripcion', actionType: 'ADD_MEDICAMENTO', details: { nombre: medicamento.nombre_comercial } })
  }
  ...
  const quitarMedicamento = (index: number) => {
    ...
    logAction({ action: 'RESOURCE_ACCESS', resourceType: 'prescripcion', actionType: 'REMOVE_MEDICAMENTO', details: { nombre: medicamento.nombre } })
  }
  ...
  logAction({ action: 'RESOURCE_ACCESS', resourceType: 'prescripcion', actionType: 'CREATE', details: { medicamentos: medicamentosSeleccionados.length } })
    onClose()
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="relative">
          <Textarea
            placeholder="Describe el diagnóstico médico o usa el micrófono para dictar..."
            value={diagnostico}
          ...
          <div className="absolute top-2 right-2 flex space-x-2">
            <Button type="button" variant="outline" size="sm" onClick={toggleVoz} className={isListening ? 'bg-red-50 text-red-600' : ''}>
              {isListening ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}
            </Button>
            <Button type="button" variant="outline" size="sm" disabled={corrigiendo} onClick={aplicarCorreccionGramatical}>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {diagnosticoTags.map((tag) => (
            <button
              key={tag}
              type="button"
              onClick={() => setDiagnostico((d) => (d ? d + ' | ' + tag : tag))}
              className="px-2 py-1 rounded-full text-xs bg-primary/10 text-primary hover:bg-primary/20 transition"
            >
              {tag}
            </button>
          ))}
        ...
        <div className="flex items-center justify-between mt-1">
          <p className="text-xs text-gray-500">
            {vozSoportada ? (isListening ? 'Escuchando...' : 'Micrófono disponible') : 'Voz no soportada'}
          </p>
          {autoCorrecciones > 0 && (
            <p className="text-xs text-green-600">Correcciones aplicadas: {autoCorrecciones}</p>
          )}
        </div>
      </div>
  ...
  const renderPaso2 = () => (
    <div className="space-y-6">
      ...
      <div className="pt-2 flex justify-end">
        <Button type="button" variant="outline" size="sm" onClick={() => setVistaPreviaActiva(v => !v)}>
          {vistaPreviaActiva ? 'Ocultar Vista Previa' : 'Ver Vista Previa'}
        </Button>
      </div>
      {vistaPreviaActiva && (
        <div className="mt-4">
          <PrescriptionPreview
            ...
            medicamentos={medicamentosSeleccionados.map((m, i) => ({ ...m, index: i }))}
